...
def model_state() -> dict:
    """Snapshot of the current download state for the frontend/settings."""
    if MODEL_STATUS["state"] in ("idle", "ready") and os.path.exists(
        model_path(MODEL_STATUS["model_key"] or DEFAULT_MODEL_KEY)
    ):
        return {**MODEL_STATUS, "state": "ready"}
    return dict(MODEL_STATUS)


def _update_progress(model_key: str, done: int, total: int) -> None:
    """Streamed-download progress -> MODEL_STATUS."""
    MODEL_STATUS["model_key"] = model_key
    MODEL_STATUS["bytes_done"] = done
    MODEL_STATUS["bytes_total"] = total
...
def download_model(model_key: str = DEFAULT_MODEL_KEY) -> dict:
    ...
    if _already_installed(model_key):
        ...
        MODEL_STATUS["bytes_done"] = MODELS[model_key]["size"]
        return dict(MODEL_STATUS)

    needed = MODELS[model_key]["size"] + SPACE_MARGIN_BYTES
    if _free_space(models_dir()) < needed:
        MODEL_STATUS["state"] = "error"
        MODEL_STATUS["error"] = "Not enough free disk space to download the model."
        raise RuntimeError(MODEL_STATUS["error"])

    try:
        MODEL_STATUS["state"] = "downloading"
        path = _stream_download(model_key)
        MODEL_STATUS["state"] = "verifying"
        MODEL_STATUS["bytes_done"] = os.path.getsize(path)
        MODEL_STATUS["state"] = "ready"
...
def _stream_download(model_key: str) -> str:
    ...
    with an HTTP Range request, and verifies the final size.
    """
    spec = MODELS[model_key]
